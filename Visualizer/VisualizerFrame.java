    ...
    public void reDrawArray(Integer[] squares, int working, int comparing, int reading){
        ...
        for(int i = 0; i<squarePanels.length; i++){
            ...
            if (i == working){
              ...
              // Add the number as a JLabel
        JLabel valueLabel = new JLabel(String.valueOf(squares[i]));
        valueLabel.setForeground(Color.white); // Text color
        valueLabel.setHorizontalAlignment(JLabel.CENTER); // Center text on the bar
        squarePanels[i].setLayout(new BorderLayout());
        squarePanels[i].add(valueLabel, BorderLayout.NORTH); // Add label to the top of the bar
